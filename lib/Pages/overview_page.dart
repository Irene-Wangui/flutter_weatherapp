...
class _OverviewPageState extends State<OverviewPage> {
  ...
  Future<TomorrowIoWeather> fetchWeatherData(String location) async {
    String url =
        "https://api.tomorrow.io/v4/weather/forecast?location=$location&apikey=dCb0TVRqI4c8j8BvE68QAehCAMcGSszN";
    log("url: $url");
    try {
          ...
          actions: [
            IconButton(
              ...
              onPressed: () async {
                ...
                if (result != null) {
                  //update the state with selected location details
                  setState(() {
                    var selectedLocationName = result.displayName;
                    var selectedLat = double.parse(result.lat);
                    var selectedLon = double.parse(result.lon);
                  });
                  // Fetch weather data using the new location
                  futureweather = fetchWeatherData(result.displayName);
                }
              },
            )
          ],
